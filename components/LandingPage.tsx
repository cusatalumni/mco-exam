...
const FeatureCard = ({ icon: Icon, title, children }: { icon: React.ElementType, title: string, children: React.ReactNode }) => (
    <div className="bg-white p-8 rounded-xl shadow-lg">
        <div className="flex items-center mb-3">
            <Icon className="text-cyan-600 mr-4" size={32} />
...
const LandingPage: React.FC = () => {
    ...
    const handleStartPractice = (examId: string) => {
        if (!user) {
            ...
            const practiceTestUrl = `${loginUrl}?redirect_to=${encodeURIComponent('/test/' + examId)}`;
            window.location.href = practiceTestUrl;
            return;
        }
    ...
    return (
        <div className="space-y-16 sm:space-y-24">
            {/* Hero Section */}
            <section className="text-center py-12 sm:py-16 bg-white rounded-xl shadow-lg">
                ...
                <p className="text-lg sm:text-xl text-slate-600 max-w-3xl mx-auto mb-8">
                    Our platform offers comprehensive practice exams to help you ace your certification.
                </p>
                {user ? (
                    <Link
                        to="/dashboard"
